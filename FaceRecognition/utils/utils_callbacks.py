...
class CallBackVerification(object):
    ...
    def __call__(self, num_update, backbone: torch.nn.Module, force=False):
        run_test = self.rank == 0 and num_update > 0 and num_update % self.frequent == 0
        if run_test or force:
            logging.info(f"======== Evaluation ========")
            if force:
                logging.info(f"=> End of Epoch Test @ Step {num_update}.")
            backbone.eval()
